...
% 'MARperiodic', [ ...
% 1   40  20  200     2.5     1/(60*60*24)*10; ...
% 2   40  20  200     2.5     1/(60*60*2)*10; ...
% 3   15  20  200     2.5     1/(60*60)*10; ...
% 4   5   20  200     2.5     1/(60*30)*10; ...
%     ] ...
% );
...
Parameters.traffic = struct( ...
    ...
    'exception',		    0,	 ... % Model exception (beta traffic)
    'poisson',              1,   ... % Model regular (poisson traffic)
...
% Initialize
REPS          = 10;
Nd_vector     = 52200;%repmat(1000,1,10);%[0.5,1,2.5,5]*1000;               %Number of devices
Nd_vector     = sort(Nd_vector,'descend');
Na_vector     = 1304; %aggregators/clusters when agg is OFF
Parameters.maxReports = 100;
...
%     simParameters.Traffic.MARperiodic(6) = lambda(i);
    parbuf = struct;

    for j=1:REPS
        % Run simulation
        [USERS,usedPRBs,dataPRBs,activeAgg, outParameters] = worker(Na_vector, Nd_vector(i), simParameters);
...
% probCollision = collidedMSG3/(collidedMSG3 + correctMSG3)
% outage = noUnf/(noDone + noUnf)
toc
