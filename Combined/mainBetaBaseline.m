...
for i = 1:length(Nd_vector)
    ...
    for j=1:REPS
        % Run simulation
        [USERS,usedPRBs,dataPRBs,activeAgg,MacFails,PhyFails,noErrors, outParameters] = worker(Na_vector, Nd_vector(i), Parameters);
...
%         efficiency(i)   = efficiency(i) + parbuf.efficiencyUL;
        PhyFailures(i)  = PhyFailures(i) + PhyFails;
        MacFailures(i)  = MacFailures(i) + MacFails;
        numberErrs(i)   = numberErrs(i) + noErrors;
...
noUnf = noUnf/REPS;
noDone = noDone/REPS;
% for k = 1:length(Na_vector)
%     usedRBsSorted(k,:) = cell2mat(usedRBs(k))/REPS;
% end

latencyAxis = [100:200:(10/Parameters.system.tdma)];
toc
