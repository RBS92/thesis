function [USERS_LTE, USERS, nodeAssocWith, activeAgg] = generateTraffic(USERS_LTE, USERS, Ns, Nd, Parameters)
% %% Traffic
% % tic
% Traffic = struct(...
...
activeAgg = 0;
% % activeAgg = 0;
% % if(Parameters.traffic.aggregators)  % If there are aggregators
% %% generate positions and perform association
%     
%     if(Parameters.Aggregators.distro == 0)
%         posAggregators = SpatialModel_PPP(Parameters.traffic.cellRadius,numberAggregators);
%     else
%         posAggregators = SpatialModel_Aggr(Parameters.traffic.cellRadius,numberAggregators,Parameters.traffic.gridInterval);
%     end
%     [distToAgg, nodeAssocWith] = nearestAssocDeviceClusterOpt(posDevices, posAggregators, numberDevices, numberAggregators);
%     
%     activeAgg = sum(unique(nodeAssocWith)>0);
% 
% %% Store Results in USERS
%     for i=1:numberAggregators
%         USERS(i).pos = posAggregators(i,:); % store position. Takes value of gateway (i.e., Aggregator to which is connected).
...
%             arrivals = nonzeros(validatedArrivals(j,:));
%             typeArrivals= idArrivals(validatedArrivals(j,:)>0);
%             data = validData(validatedArrivals(j,:)>0);
%             
...
%           
%             u = [];
%             if(Parameters.Aggregators.link == 1)
%                 for l = 1:length(arrivalsBuf)
%                     [peTX,~] = phyAggUE(distances(l),Parameters);
%                     if(rand<peTX)
%                         u = [u,l];
%                     end
%                 end
...
%                 arrivalsBuf(u) = [];
%                 typeOfArrivalBuf(u) = [];
%                 USERS(i).failedToAgg = USERS(i).failedToAgg + length(u);
%             end
% 
%         end
%         
...
%         distances = distances(bufIndex);
%         
%         % [arrivalsBuf,typeOfArrivalBuf, failedArrivals, failedType] = ...
%         % UEtoAggComm(arrivalsBuf, typeOfArrivalBuf, distances, Parameters);
% 
%         if(length(arrivalsBuf)>Parameters.maxReports)
%             warning('maxReports is too small')
%         end
% 
%         if (arrivalsBuf)
% %             USERS(i).reportEarlyFailures(1:length(failedArrivals)) = failedArrivals;
% %             USERS(i).reportEarlyFailureTypes(1:length(failedType)) = failedType;
...
%             USERS(i).nextRACH = arrivalsBuf(1) + find( RAO(arrivalsBuf(1):arrivalsBuf(1)+20) > 0,1) - 1; % nextRACH should match a RAO
%             USERS(i).nextPreamble = Preambles(i,1);
%         end
%     end
% else %if there aren't aggregators
...
posAggregators = SpatialModel_OPTIMAL(Parameters.traffic.cellRadius,numberAggregators);
% posAggregators = SpatialModel_PPP(Parameters.traffic.cellRadius,numberAggregators);
[distToAgg, nodeAssocWith] = nearestAssocDeviceClusterOpt(posDevices, posAggregators, numberDevices, numberAggregators);

for i=1:numberAggregators
    USERS_LTE(i).pos = posAggregators(i,:); % store position. Takes value of gateway (i.e., Aggregator to which is connected).
end
%         USERS(i).pos = posAggregators(i,:); % store position. Takes value of gateway (i.e., Aggregator to which is connected).
